<template>
  <div class="auth-page">
    <div class="auth-card fade-up">
      ...
      <p class="sub">Start processing files for free</p>

      <div class="field">
        <label>Username</label>
        <input v-model="username" type="text" placeholder="yourname" @keyup.enter="submit" />
      </div>
      <div class="field">
        <label>Email</label>
      ...
      <div class="field">
        ...
        <input v-model="password" type="password" placeholder="Min. 8 characters" @keyup.enter="submit" />
      </div>
...
async function submit() {
  ...
  try {
    ...
    error.value = d?.email?.[0] || d?.username?.[0] || d?.password?.[0] || 'Registration failed.'
  } finally {
    loading.value = false
  }
}
